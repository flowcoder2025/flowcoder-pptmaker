...
- **Pro 플랜 초과 슬라이드 비용 시스템 구현**
  ...
  - constants/subscription.ts에 getExtraSlideCount(), getExtraSlideCreditCost() 함수 추가
  - store/presentationStore.ts에 초과 슬라이드 크레딧 차감 로직 추가
  - input 페이지에 초과 슬라이드 비용 실시간 표시

- **크레딧 페이지 초과 슬라이드 비용 안내 추가**
  - 크레딧 사용량 안내 섹션에 "초과 슬라이드" 카드 추가
  - Pro 플랜 20장 초과 시 2 크레딧/장 비용 표시
  - 그리드 레이아웃 3열로 확장

- **Pro 플랜 20장 무제한 무료 안내 강화**
  - input 페이지 전반에 걸쳐 Pro 플랜 슬라이드 정책 명확히 표시
    - 슬라이드 생성 카드: "20장까지 무제한 무료!" 강조
    - 크레딧 잔액 카드: "슬라이드 20장까지 무료" 안내
    - 슬라이더 값에 (무료)/(+N장 추가) 실시간 표시
    - 슬라이더 하단에 "20장 무료" 레이블 추가
  - subscription 페이지 업데이트
    - PlanCard: "슬라이드 20페이지 무료 (최대 50)"
    - 비교표: Pro 플랜 "20페이지 무료 (최대 50)"

- **홈페이지 플랜 프리뷰 섹션 추가**
  - 무료/Pro/Premium 3단 플랜 카드 추가
  - Pro 플랜 "20장 무료 (최대 50장)" 강조 표시
  - Pro 플랜 핵심 혜택: 심층검색·고품질 생성 무제한, 광고·워터마크 제거
  - Premium 플랜 출시 예정 표시 및 알림 신청 버튼
  - 하단에 "21장부터 2 크레딧/장" 추가 설명

### 🐛 Fixes

#### 2025-12-26
- **PDF/PPTX 다운로드 시 슬라이드 푸터 잘림 문제 수정**
  - 문제: vh/vw CSS 단위가 브라우저 뷰포트 기준으로 계산되어 푸터가 잘림
  - html2canvas → html-to-image 라이브러리로 전환
  - iframe 기반 렌더링으로 vh/vw가 슬라이드 크기(1200×675) 기준으로 계산되도록 수정
  - pixelRatio: 2 적용으로 고해상도 출력 (2400×1350)
  - JPEG 품질 0.92로 파일 크기 최적화

- **Pro 플랜 슬라이더 50장까지 조절 가능하도록 수정**
  - useEffect에서 Pro 플랜 최대값을 50으로 변경
  - 기존에는 planMaxSlides(20)로 제한되어 슬라이더가 20 초과 불가
